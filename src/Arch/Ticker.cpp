...
struct Ticker : Module
{
	...
	Ticker()
	{
		...
		configInput(MSR_GATE_LEN_INPUT, "Gate Length Voltage (0..10V)");
		...
		for (i = 0; i < 4; i++)
		{
			...
			configParam(CLK_GATE_LEN_PARAMS + i, MIN_GATE_LEN, MAX_GATE_LEN, 50.f, "Gate Length", "%");
		...
		for (i = 0; i < 4; i++)
		{
			...
			fmt = name;
			configInput(CLK_GATE_LEN_INPUTS + i, fmt);
	...
	void process(const ProcessArgs &args) override
	{
		...
		if (getInput(MSR_GATE_LEN_INPUT).isConnected())
			msr_Gate_Len = MIN_GATE_LEN + (MAX_GATE_LEN - MIN_GATE_LEN) * getInput(MSR_GATE_LEN_INPUT).getVoltage() * 0.1f;
		else
		...
		if (getInput(CLK_GATE_LEN_INPUTS + 0).isConnected())
			clk1_Gate_Len = MIN_GATE_LEN + (MAX_GATE_LEN - MIN_GATE_LEN) * getInput(CLK_GATE_LEN_INPUTS + 0).getVoltage() * 0.1f;
		else
			clk1_Gate_Len = (int)getParam(CLK_GATE_LEN_PARAMS + 0).getValue();
		...
		// Gate Length Data = 10V mapped to range 5-95%
		if (getInput(CLK_GATE_LEN_INPUTS + 1).isConnected())
			clk2_Gate_Len = MIN_GATE_LEN + (MAX_GATE_LEN - MIN_GATE_LEN) * getInput(CLK_GATE_LEN_INPUTS + 1).getVoltage() * 0.1f;
		else
			clk2_Gate_Len = (int)getParam(CLK_GATE_LEN_PARAMS + 1).getValue();
		...
		// Gate Length Data = 10V mapped to range 5-95%
		if (getInput(CLK_GATE_LEN_INPUTS + 2).isConnected())
			clk3_Gate_Len = MIN_GATE_LEN + (MAX_GATE_LEN - MIN_GATE_LEN) * getInput(CLK_GATE_LEN_INPUTS + 2).getVoltage() * 0.1f;
		else
			clk3_Gate_Len = (int)getParam(CLK_GATE_LEN_PARAMS + 2).getValue();
		...
		// Gate Length Data = 10V mapped to range 5-95%
		if (getInput(CLK_GATE_LEN_INPUTS + 3).isConnected())
			clk4_Gate_Len = MIN_GATE_LEN + (MAX_GATE_LEN - MIN_GATE_LEN) * getInput(CLK_GATE_LEN_INPUTS + 3).getVoltage() * 0.1f;
		else
			clk4_Gate_Len = (int)getParam(CLK_GATE_LEN_PARAMS + 3).getValue();
		...
		if (is_Running)
		{
			...
			getOutput(CLK_GATE_OUTPUTS + 1).setVoltage((clk1_GateState) ? 10.f : 0.f);
			getOutput(CLK_TRIGGER_OUTPUTS + 1).setVoltage((clk1_TrgState) ? 10.f : 0.f);
			getLight(CLK_PULSE_LIGHTS + 1).setBrightnessSmooth(clk1_Gate_Voltage > 0.f, args.sampleTime);
			...
			getOutput(CLK_GATE_OUTPUTS + 2).setVoltage((clk1_GateState) ? 10.f : 0.f);
			getOutput(CLK_TRIGGER_OUTPUTS + 2).setVoltage((clk1_TrgState) ? 10.f : 0.f);
			getLight(CLK_PULSE_LIGHTS + 2).setBrightnessSmooth(clk1_Gate_Voltage > 0.f, args.sampleTime);
			...
			getOutput(CLK_GATE_OUTPUTS + 3).setVoltage((clk1_GateState) ? 10.f : 0.f);
			getOutput(CLK_TRIGGER_OUTPUTS + 3).setVoltage((clk1_TrgState) ? 10.f : 0.f);
			getLight(CLK_PULSE_LIGHTS + 3).setBrightnessSmooth(clk1_Gate_Voltage > 0.f, args.sampleTime);
		}
		else // Not running
		{
...
struct TickerWidget : ModuleWidget
{
	TickerWidget(Ticker *module)
	{
		...
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(74.0, 26.75)), module, Ticker::MSR_GATE_LEN_INPUT));
		...
		// Gate Length
		addParam(createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(62.5, 46.5)), module, Ticker::CLK_GATE_LEN_PARAMS + 0));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(72.0, 46.5)), module, Ticker::CLK_GATE_LEN_INPUTS + 0));
		...
		// Gate Length
		addParam(createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(62.5, 63.5)), module, Ticker::CLK_GATE_LEN_PARAMS + 1));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(72.0, 63.5)), module, Ticker::CLK_GATE_LEN_INPUTS + 1));
		...
		// Gate Length
		addParam(createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(62.5, 80.5)), module, Ticker::CLK_GATE_LEN_PARAMS + 2));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(72.0, 80.5)), module, Ticker::CLK_GATE_LEN_INPUTS + 2));
		...
		// Gate Length
		addParam(createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(62.5, 97.5)), module, Ticker::CLK_GATE_LEN_PARAMS + 3));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(72.0, 97.5)), module, Ticker::CLK_GATE_LEN_INPUTS + 3));
